				 ...
				 struct nlattr **tb)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Drv Event %d (%s) received for %s",
		   cmd, nl80211_command_to_string(cmd), bss->ifname);
	wpa_msg(drv->ctx, MSG_INFO, "HELGA - nl80211: Drv Event %d (%s) received for %s",
		   cmd, nl80211_command_to_string(cmd), bss->ifname);
	...
	case NL80211_CMD_TRIGGER_SCAN:
		...
		wpa_msg(drv->ctx, MSG_INFO, "HELGA - nl80211: Scan trigger - EVENT_SCAN_STARTED");
		drv->scan_state = SCAN_STARTED;
		wpa_supplicant_event(drv->ctx, EVENT_SCAN_STARTED, NULL);
		break;
	case NL80211_CMD_START_SCHED_SCAN:
		wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: Sched scan started");
		wpa_msg(drv->ctx, MSG_INFO, "HELGA - nl80211: Sched scan started - SCHED_SCAN_STARTED");
		drv->scan_state = SCHED_SCAN_STARTED;
		break;
	case NL80211_CMD_SCHED_SCAN_STOPPED:
		wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: Sched scan stopped");
		wpa_msg(drv->ctx, MSG_INFO, "HELGA - nl80211: Sched scan stopped - SCHED_SCAN_STOPPED");
	...
	case NL80211_CMD_NEW_SCAN_RESULTS:
		...
		wpa_msg(drv->ctx, MSG_INFO,
			"HELGA - nl80211: New scan results available - SCAN_COMPLETED");
	...
	case NL80211_CMD_SCHED_SCAN_RESULTS:
		...
		wpa_msg(drv->ctx, MSG_INFO,
			"HELGA - nl80211: New sched scan results available - SCHED_SCAN_RESULTS");
		drv->scan_state = SCHED_SCAN_RESULTS;
		send_scan_event(drv, 0, tb);
		break;
	case NL80211_CMD_SCAN_ABORTED:
		wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: Scan aborted");
		wpa_msg(drv->ctx, MSG_INFO, "HELGA - nl80211: Scan aborted - SCAN_ABORTED");
